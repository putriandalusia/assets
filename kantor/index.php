...
?>
<?php if($_SESSION["akses"] == "marketing" OR $_SESSION["akses"] == "offline"){
	echo '
                        <li>
...
<?php $tawar =0;
	...
	while ($dat = mysql_fetch_array($qdat)){
		...
		if($dat['lunas'] == 'lunas'){
			...
			$alert = $alert.'<div class="alert alert-danger">
                            Anda mendapatkan order dari '.$dat['dealer_view'].' utk '.$dat['kategori'].'-'.$dat['id_hwn'].' <a href="https://api.whatsapp.com/send?phone='.$wa_no.'&text=Assalamualaikum%2C+saya+marketing+SQ+terima+kasih+telah+memesan+http%3A%2F%2Fshowroomqurban.com%2F%3Fq%3D'.$dat['id_hwn'].'" type="button" class="btn btn-danger">FollowUp</a>
                            </div>';
			}
		}else{
			$tawar++;
	...
	<td>'.$dat7['showroom_view'].'</td>
		...
		';
		$tabad = $tabad.$tabi;
		$wa_r =$wa_r.$tnd.' '.$dat7['kategori'].'-'.$dat7['id_hwn'].' ✏'.$dat7['showroom_view'].' '.number_format($dat7['harga_lama'],0).'| _'.$dat7['pemilik'].'_ &#13;&#10;';
	}
	$total = $keep+$lunas;
