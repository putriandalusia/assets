...
$gbc = $_GET['q'];
$qco = mysql_query("SELECT `foto` FROM `$db`.`datil_hewan` WHERE `id_hwn` = '$gbc'");
$cekco = mysql_num_rows($qco);
if($cekco > 0){
$slide ='';$itema = "item active";
while($dt_gco = mysql_fetch_array($qco)){
	...
	if($qre == true){
		...
		<div class="col-md-6 col-md-offset-3">
	        					<div class="text-center">
	        						...
	        						<p class="description">
	        							Anda adalah pembeli yang bijak, setelah anda mengenal salah satu markting kami. Sekarang klik tombol dibawah ini untuk memesan kendaraan akherat anda :)
										<a href ="https://api.whatsapp.com/send?phone='.$ref_no.'&text=Assalamualaikum%2C+apakah+saya+bisa+pesan+hewan+qurban+ini+http%3A%2F%2Fshowroomqurban.com%2F%3Fq%3D'.$id.'">
...
if($stal == NULL ){
	if(isset($_GET['cq'])){
		$get5 = $_GET['cq'];
		$qre5 = mysql_fetch_array(mysql_query("SELECT * FROM `$db`.`akses` WHERE `username` ='$get5' AND `email` IS NOT NULL"));
